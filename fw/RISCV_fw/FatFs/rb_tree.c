...
rb_red_blk_tree* RBTreeCreate( rb_red_blk_tree* newTree,
  ...
  newTree->MallocFunc= MallocFunc;


  /*  see the comment in the rb_red_blk_tree structure in red_black_tree.h */
  /*  for information on nil and root */
  temp=newTree->nil= (rb_red_blk_node*) newTree->MallocFunc(1);
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= (rb_red_blk_node*) newTree->MallocFunc(1);
...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  x=(rb_red_blk_node*) tree->MallocFunc(0);
  x->key=key;
  //x->info=info;
