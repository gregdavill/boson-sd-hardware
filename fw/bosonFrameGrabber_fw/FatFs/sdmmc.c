...
#define CS_H() gpio_out |= 0x04  /* Set MMC CS "high" */
#define CS_L() gpio_out &= ~0x04 /* Set MMC CS "low" */

//#define ENABLE_READ_CACHE (1)

DRESULT disk_read_ (BYTE pdrv, BYTE* buff, DWORD sector, UINT count);
DRESULT disk_write_ (BYTE pdrv, const BYTE* buff, DWORD sector, UINT count);
...
)
{
	...
	do
	{
		...
		*buff++ = spi_dat; /* Store a received byte */
	} while (--bc);
}
...
#ifdef ENABLE_READ_CACHE
	SD_cache_init(0x04000000 + 0x80000, 128); /* Init buffer 1Mb into RAM*/
#endif
...
{
#ifdef ENABLE_READ_CACHE
	if(SD_cache_read(buff, sector, count)){
		return RES_OK;
	}
#endif

	/* Cache Miss */
	DRESULT res = disk_read_(drv, buff, sector, count);

#ifdef ENABLE_READ_CACHE
	SD_cache_create(buff, sector, count);
#endif
...
{
#ifdef ENABLE_READ_CACHE
	SD_cache_update(buff, sector, count);
#endif

	/* Cache Miss */
	return disk_write_(drv, buff, sector, count);
}
